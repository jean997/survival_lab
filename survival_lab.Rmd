...
.panelset[
.panel[.panel-name[R Code]
```{r kmsex, fig.show='hide', fig.align='center', fig.height=6, fig.width=12}
...
- Median survival is 156 days longer in females than in males. 
  + Note that this is an *associational* description. 
...
- In some cases, it might be useful to display survival curves estimated from the fitted model. 

- Since we have many covariates, we need to choose some covariate values for our curves. 
...
- We will plot survival curves for male and female patients of median age and
  + Have a Karnofsky score of 90 or
  + Have a Karnofsky score of 70
  + (So four curves)
...
- There are three strategies for accommodating non-proportional hazards within Cox regression:

1. Convert a time-fixed covariate into a time-varying covariate. 

2. Stratify the model on a single discrete covariate. 
...
- Suppose $V$ is a categorical variable with $K$ levels. We want to fit

$$\lambda(t; X, V, \beta) = \lambda_{0, V}(t)exp(X^\top \beta)$$
- So we will estimate $K$ different (non-parametric) baseline hazard functions. 
...
$$\lambda(t; \tilde{X}, \beta) = \lambda_0(t) \exp(\beta\tilde{X}(t))$$
...
- We can try to use th plot from `cox.zph` to guess a good transformation. 
...
+ In our case, I will try a piecewise constant function with two knots, one at 200, and one at 450 days
...
- To fit a piece-wise model with three breaks, we need to add two time-varying covariates. 
...
zp <- cox.zph(coxfit1, transform = "identity")
plot(zp[3])
b <- function(t){coxfit4$coefficients[3] + 
                 coxfit4$coefficients[4]*(t > 200) + 
                 coxfit4$coefficients[5]*(t > 450)}
curve(b(x), add = TRUE, col = "blue")
```
...
- Seems like the time-varying model is doing better. 
